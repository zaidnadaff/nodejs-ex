class RollingSizeOptions {
  ...
  static assert(size_threshold) {
    if (typeof size_threshold !== "number" || size_threshold < this.OneKB) {
      throw new Error(
        `size_threshold must be atleast one KB. unsupported param ${JSON.stringify(
          size_threshold
        )}`
      );
    }
...
class RollingTimeOptions {
  ...
  static assert(time_option) {
    if (
      ...
      throw new Error(
        `time_option must be an instance of RollingTimeOption. Unsupported param ${JSON.stringify(
          time_option
        )}`
      );
    }
